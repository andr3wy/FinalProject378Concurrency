...
do
    ...
    do
        ...
        lock_stack=$(echo "$command_output" | grep -oE 'LockStack: [0-9]+')
        free_stack=$(echo "$command_output" | grep -oE 'FreeStack: [0-9]+')
        lock_queue=$(echo "$command_output" | grep -oE 'LockQueue: [0-9]+')
        free_queue=$(echo "$command_output" | grep -oE 'FreeQueue: [0-9]+')
        lock_map=$(echo "$command_output" | grep -oE 'LockMap: [0-9]+')
        free_map=$(echo "$command_output" | grep -oE 'FreeMap: [0-9]+')
        ...
        if [ -n "$free_stack" ]; then
            total_time_free_stack=$(awk "BEGIN { print $total_time_free_stack + ${free_stack#FreeStack: } }")
        fi
        if [ -n "$lock_queue" ]; then
            total_time_lock_queue=$(awk "BEGIN { print $total_time_lock_queue + ${lock_queue#LockQueue: } }")
        fi
        if [ -n "$free_queue" ]; then
            total_time_free_queue=$(awk "BEGIN { print $total_time_free_queue + ${free_queue#FreeQueue: } }")
        fi
        if [ -n "$lock_map" ]; then
            total_time_lock_map=$(awk "BEGIN { print $total_time_lock_map + ${lock_map#LockMap: } }")
        fi
        if [ -n "$free_map" ]; then
            total_time_free_map=$(awk "BEGIN { print $total_time_free_map + ${free_map#FreeMap: } }")
        fi
        # else
    ...
    # echo "$total_time_lock_stack" >> "$o_lock_stack"
    # echo "$total_time_free_stack" >> "$o_free_stack"
    # echo "$total_time_lock_queue" >> "$o_lock_queue"
    # echo "$total_time_free_queue" >> "$o_free_queue"
    # echo "$total_time_lock_map" >> "$o_lock_map"
    # echo "$total_time_free_map" >> "$o_free_map"
